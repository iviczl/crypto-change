...
export const userList = createAsyncThunk<IUser[]>('users/list', () =>
  getUsers()
)
export const login = createAsyncThunk<
  IUser,
  { userName: string; password: string }
>('user/login', (u) => logIn(u.userName, u.password))
export const logout = createAsyncThunk('user/logout', () => logOut())
export const addUserCurrency = createAsyncThunk(
  'user/addCurrency',
  async (c: Currency, { getState }) => {
...
export const removeUserCurrency = createAsyncThunk(
  'user/removeCurrency',
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(login.fulfilled, (state, action) => {
        ;(state.status = FeatureState.SUCCEEDED), (state.user = action.payload)
      })
      ...
      .addCase(userList.fulfilled, (state, action) => {
        ;(state.status = FeatureState.SUCCEEDED), (state.users = action.payload)
      })
